## Liniowe modele mieszane (MLM): Porównanie efektywności algorytmów w językach Python i R
--- 

### Spis treści
- [Liniowe modele w statystyce](#liniowe-modele-w-statystyce)
  * [Ogólne modele liniowe (GLM) - estymacja parametrów i zastosowanie](#og-lne-modele-liniowe--glm----estymacja-parametr-w-i-zastosowanie)
  * [Mieszane modele liniowe (MLM) - estymacja parametrów i zastosowanie](#mieszane-modele-liniowe--mlm----estymacja-parametr-w-i-zastosowanie)
- [Języki Python i R](#j-zyki-python-i-r)
  * [Porównanie efektywności, zastostowań i popularności](#por-wnanie-efektywno-ci--zastostowa--i-popularno-ci)
  * [Impementacja metod statystycznych](#impementacja-metod-statystycznych)
- [Porównanie efektywności algorytmów MLM](#por-wnanie-efektywno-ci-algorytm-w-mlm)
  * [Metodyka](#metodyka)
  * [Wyniki](#wyniki)
    + [Zbiór danych "cows"](#zbi-r-danych--cows-)
  * [Wnioski](#wnioski)

---

### Liniowe modele w statystyce
---
#### Ogólne modele liniowe (GLM) - estymacja parametrów i zastosowanie
---
#### Mieszane modele liniowe (MLM) - estymacja parametrów i zastosowanie

---

### Języki Python i R
#### Porównanie efektywności, zastostowań i popularności
#### Impementacja metod statystycznych

---

### Porównanie efektywności algorytmów MLM
#### Metodyka
#### Wyniki
##### Zbiór danych "cows"
#### Wnioski
